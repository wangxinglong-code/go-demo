...
func main() {
	...
	path := "D:\\workspace\\go\\src\\go-demo\\models"
	// #################### Config #####################
...
func (table *{table_name_title}) DbAliasName() string {
	return "go_demo"
}
...
func (table *{table_name_title}) DbNode() int {
	return 0
}

func (table *{table_name_title}) Get(id int) (*{table_name_title}, error) {
	var m {table_name_title}
	db, err := mysql.GetConn(table.DbAliasName(), table.DbNode(), true)
	if err != nil {
		return nil, err
	}
	if err := db.Table(table.TableName()).Where("id = ?", id).First(&m).Error; err != nil {
		return nil, err
	}
	return &m, nil
}

func (table *{table_name_title}) Create(m {table_name_title}) error {
	db, err := mysql.GetConn(table.DbAliasName(), table.DbNode(), true)
	if err != nil {
		return err
	}
	if err := db.Table(table.TableName()).Create(&m).Error; err != nil {
		return err
	}
	return err
}
...
func (table *{table_name_title}) Save(m {table_name_title}) error {
	...
	if err := db.Table(table.TableName()).Save(&m).Error; err != nil {
		return err
	}
	return err
}
...
func (table *{table_name_title}) Delete(m *{table_name_title}) error {
	...
	if err := db.Table(table.TableName()).Where("id = ?", m.Id).Delete(&m).Error; err != nil {
		return err
	}
	return err
}
